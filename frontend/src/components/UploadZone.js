...
export default function UploadZone({ files, setFiles }) {
  ...
  const ACCEPTED_EXTENSIONS = ['.pdf', '.zip', '.docx', '.doc', '.xlsx', '.xls', '.txt', '.odt', '.rtf', '.eml',
    ...
    <div data-testid="upload-zone-container">
      ...
      >
        ...
        <input
          ...
          type="file"
          multiple
          accept=".pdf,.zip,.docx,.doc,.xlsx,.xls,.txt,.odt,.rtf,.eml,.jpg,.jpeg,.png,.webp,.heic,.heif,.tiff,.tif,.bmp"
