...
import { ArrowUpDown, Search, Download, Loader2, Settings2, Layers, List } from 'lucide-react';
...
export default function AllContactsTable() {
  ...
  const activeColumns = grouped ? GROUPED_COLUMNS : ALL_FIELDS;
  ...
  const downloadGroupedCSV = () => {
    ...
    const headers = GROUPED_COLUMNS.map(c => c.label);
    const esc = (v) => { const s = String(v ?? ''); return s.includes(',') || s.includes('"') || s.includes('\n') ? `"${s.replace(/"/g, '""')}"` : s; };
    const fmtDate = (iso) => { if (!iso) return ''; try { return new Date(iso).toLocaleDateString('en-US'); } catch { return iso; } };
    const rows = groupedData.map(c => GROUPED_COLUMNS.map(col => col.key === 'import_date' ? fmtDate(c[col.key]) : c[col.key]).map(esc).join(','));
      ...
      <Dialog open={showExport} onOpenChange={setShowExport}>
        <DialogContent className="bg-[#111827] border-slate-800 text-slate-200 max-w-sm">
          ...
          <div className="flex items-center justify-between pt-2 border-t border-slate-800">
            <button onClick={() => setExportFields(exportFields.length === ALL_FIELDS.length ? [] : ALL_FIELDS.map(f => f.key))}
